...
// One curated file: notes.json (week → workstream → one sentence).
...
//
// updates.json used to sit beside it, carrying the UI-improvements band. The
// band is now a tile with a measured number (see view.mjs), so its prose moved
// into notes.json under `uiImprovements` and the second curated file went away.
